...
/*   Updated: 2018/12/20 14:32:54 by msnow-be         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void	put_fractional_part(double fract_part, int precision)
{
	while (precision--)
	{
		fract_part *= 10;
...
void	get_int_part_and_exponent(double *n, int *int_part,
...
int		put_double(double n, t_format format)
{
	...
	get_int_part_and_exponent(&n, &int_part, &exponent, format);
	...
	if (format.precision > 0)
		put_fractional_part(n - (int)n, format.precision);
	if (format.scientific_notation)
	{
		write(1, "e", 1);
...
void	update_precision_by_significance(double n, int *precision)
{
	double		fract_part;
	int			confirmed_precision;

	confirmed_precision = 0;
	fract_part = n - (long long)n;
	while (confirmed_precision < *precision)
	{
		confirmed_precision++;
		fract_part *= 10.0;
		if (fract_part - (long long)fract_part > 0.99999)
			fract_part += 0.000001;
	}
	if ((long long)fract_part % 10)
		return ;
	while (((long long)fract_part != 0) && confirmed_precision > 0)
	{
		confirmed_precision--;
		if (fract_part - (long long)fract_part >= 0.5)
			break ;
